...
- The ONLY hand-edited spec files are `.ai/context.md` (the project contract) and
  ...
  GENERATED by `cargo xtask sync-ai` (crates/xtask) — NEVER edit a generated
...
- How each entry file is produced:
  ...
  - Commands (`.ai/commands/`), subagents (`.ai/agents/`), and skills (`.ai/skills/`) are
    hand-edited sources too; `cargo xtask sync-ai` fans them into every tool's format. A skill may use a
...
- To change a convention: edit `.ai/context.md` / `.ai/pipeline.md`, run `cargo xtask
...
`.ai/memory.md` · generated agent docs in sync (`cargo xtask sync-ai`, enforced by the pre-commit
...
- `.ai/memory.md` is a LOCAL, per-developer working log — it is **gitignored**, not
  ...
  automatically on `cargo xtask sync-ai`).
